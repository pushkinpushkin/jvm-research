...
## Программа исследования v1

Основные условия и ограничения измерений: [короткий протокол](RESEARCH_PROTOCOL.md).
Автоматический допуск результатов: `validation.json`; расчёты по фазам: `phase-summary.json`.

Порядок продолжения исследования: [текущее состояние](CURRENT_STATE.md), [схема результатов](RESULTS_SCHEMA.md), [решения](DECISIONS.md), [следующие этапы](BACKLOG.md). Правила адресной работы с контекстом — в [AGENTS.md](AGENTS.md).
